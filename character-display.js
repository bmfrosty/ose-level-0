...
function display0LevelCharacter(results, total, background, hitPoints, armorClass, race, name, startingGold) {
    ...
    // Check if character becomes an adventurer
    if (!hitPoints.isAdventurer) {
        let resultHtml = `
            <h2 style='text-align: center;'>0-Level Character</h2>
            <div style='text-align: center; margin-bottom: 20px;'>
                <h3>Occupation: ${background.profession}</h3>
                <h3 style='color: red;'>This character didn't become an adventurer</h3>
                <p>Hit Point Roll: ${hitPoints.roll} + CON modifier (${results.find(r => r.ability === "CON").modifier}) = ${hitPoints.total}</p>
                <p><em>Characters with 0 or fewer hit points don't have what it takes to become adventurers.</em></p>
            </div>
        `;
